...
class Driver(DTROS):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown(): # change this into 1.25 memters
            ...
            if phase == 0 and not self.drive_straight(rate, starting, 0.2): # long sided D straight
                phase = 0.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
            ...
            if phase == 1 and not self.rotate(rate, starting, 0.048, direction='right'): # first turn
                phase = 1.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
            ...
            if phase == 1.5 and not self.drive_straight(rate, starting, 0.2): # long sided D straight
                phase = 2
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 2 and not self.wait(rate, t, 1):
                phase = 2.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 2.5 and not self.rotate(rate, starting, 0.048, direction='right'): # first turn
                phase = 3
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 3 and not self.wait(rate, t, 1):
                phase = 3.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            # DONE 2 SIDE

            if phase == 3.5 and not self.drive_straight(rate, starting, 0.2): # long sided D straight
                phase = 4
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
                continue

            if phase == 4 and not self.wait(rate, t, 1):
                phase = 4.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                continue

            if phase == 4.5 and not self.rotate(rate, starting, 0.048, direction='right'): # first turn
                phase = 5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
                continue

            if phase == 5 and not self.wait(rate, t, 1):
                phase = 5.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
            ...
            if phase == 5.5 and not self.drive_straight(rate, starting, 0.2): # long sided D straight
                phase = 6
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 6 and not self.wait(rate, t, 1):
                phase = 6.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 6.5 and not self.rotate(rate, starting, 0.048, direction='right'): # first turn
                phase = 7
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 7 and not self.wait(rate, t, 1):
                phase = 7.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
