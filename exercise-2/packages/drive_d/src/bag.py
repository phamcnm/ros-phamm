...
bag_location = "/home/roboticslab229r/Documents/ros-phamm/ros-phamm/exercise-2/packages/drive_d/src/move.bag"
...
world_frame_list = []
cur_theta = np.pi / 2
rotation_mat = np.array([[np.cos(cur_theta), -np.cos(cur_theta), 0], [np.cos(cur_theta), np.cos(cur_theta), 0], [0, 0, 1]])
# point_list = point_list[100:200]
for i in range(len(point_list)):
    xr = 0.0318 * point_list[i][2] / (2 * 0.7) + 0.0318 * point_list[i][1] / (2 * 0.66)  
    yr = 0
    thetar = 0.0318 * point_list[i][2] / (2 * 0.051 * 0.7) - 0.0318 * point_list[i][1] / (2 * 0.051 * 0.66)
    robot_frame = np.array([xr, yr, thetar])
    cur_theta = np.pi / 2 + thetar * 0.05
...
for dx, dy, dtheta in world_frame_list:
    theta += dtheta
    x += (dx * np.cos(theta) - dy * np.sin(theta)) * 0.5
    y += (dx * np.sin(theta) + dy * np.cos(theta)) * 0.5
