...
class Driver(DTROS):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown(): # change this into 1.25 memters
            ...
            if time.time() - self.start_time < 3.5: # wait 3 seconds until moving
            ...
            if phase == 0 and not self.drive_straight(rate, starting, 1.2): # long sided D straight
                phase = 0.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
            ...
            if phase == 1 and not self.rotate(rate, starting, 0.036, direction='right'): # first turn
                phase = 1.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
            ...
            if phase == 2 and not self.drive_straight(rate, starting, 0.8): # top D straight
                phase = 2.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 2.5 and not self.wait(rate, t, 1):
                phase = 3
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 3 and not self.curve(rate, starting, 0.5): # first curve
                phase = 3.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 3.5 and not self.wait(rate, t, 1):
                phase = 4
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 4 and not self.drive_straight(rate, starting, 0.56): # small sided D straight
                phase = 4.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 4.5 and not self.wait(rate, t, 1):
                phase = 5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 5 and not self.curve(rate, starting, 0.5): # second curve
                phase = 5.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 5.5 and not self.wait(rate, t, 1):
                phase = 6
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 6 and not self.drive_straight(rate, starting, 0.8): # bottom D straight
                phase = 6.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()

            if phase == 6.5 and not self.wait(rate, t, 1):
                phase = 7
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)

            if phase == 7 and not self.rotate(rate, starting, 0.036, direction='right'): # final turn
                phase = 7.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
                t = time.time()
                self.needs_light_update = True

            if phase == 7.5 and self.needs_light_update:
                phase = 8
                self.needs_light_update == True
                # self.state == '3'
                self.publish_light(rate, '2')

            if phase == 8 and not self.wait(rate, t, 5):
                phase = 8.5
                starting = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)     
    ...
    def curve(self, rate, starting, distance, direction='right'):
        cur_pos = 2 * 3.14159 * self._radius * (self._ticks_left - self._starting_ticks_left)
        if direction == 'right':
            message = WheelsCmdStamped(vel_left=self._vel_left * 1.1, vel_right=self._vel_right * 0.5)
        else:
