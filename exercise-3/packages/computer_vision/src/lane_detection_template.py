...
class LaneDetectionNode(DTROS):
    ...
    def detect_corner(self, white_contours, yellow_contours, side):
        ...
        #         return True
        if len(yellow_contours) == 0 or len(white_contours) == 0:
                return True
        return False
    ...
    def get_lane_error(self, white_contours, yellow_contours, side):
        ...
        x_dis = (cur - avg)
        y_dis = (max_white_point[1] - max_yellow_point[1])*(0.5-side)*(-2)
        if len(white_contours) == 0 or len(yellow_contours) == 0:
            y_dis = 0
        print((x_dis, y_dis))
        prop = (x_dis + y_dis) / self.w
        return prop
    ...
    def callback(self, msg):
        ...
        undistorted_img = undistorted_img[int(0.55 * self.h):, :]
        self.h, self.w = undistorted_img.shape[:2]
        ...
        # mask_combination = cv2.bitwise_or(white_mask, yellow_mask)
        # mask_combination = cv2.dilate(mask_combination, kernel)
        # res_all = cv2.bitwise_and(undistorted_img, undistorted_img, mask=mask_combination)

        # gray = cv2.cvtColor(res_all, cv2.COLOR_RGB2GRAY) 
        ...
        #     averaged_lines = average_slope_intercept(undistorted_img, lines)
        #     line_image = display_lines(undistorted_img, averaged_lines)
        #     combo_image = cv2.addWeighted(undistorted_img, 0.8, line_image, 1, 1)
        
        
        # contours, hierarchy = cv2.findContours(mask_combination, 
        #                                 cv2.RETR_TREE, 
        ...
        #     done = self.rotate(self.rate_wheel, self.starting_location, 0.02, direction='left')
        #     if done:
        #         self.performing_turn = False
        #     else:
        #         return
        ...
        # self.detecting_corner = self.detect_corner(white_contours, yellow_contours, self.side)
        ...
        dt = 0.01  # Time step
        error = self.get_lane_error(white_contours, yellow_contours, self.side)
